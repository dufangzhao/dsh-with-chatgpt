...
describe("connectorAction", () => {
  ...
  it("updates when the old address was reclaimed", () => {
    ...
    expect(reclaimUserMessage("DSH with ChatGPT")).toContain("删除");
    expect(reclaimUserMessage("DSH with ChatGPT")).not.toContain("Reconnect");
  });
