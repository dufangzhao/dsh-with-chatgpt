...
program
  .name("d2c")
  .description(`${PRODUCT_NAME} — ChatGPT thinks. DSH works.`)
      ...
      if (opts.json) {
        say(
          JSON.stringify({
            ...
            host: { executor: "dsh", sandboxMutation: false },
            tunnel: {
...
program
  ...
  .action(async (opts: { workspace?: string; json: boolean }) => {
    ...
    if (observation.state === "stopped") {
      ...
      else say("Bridge 未运行。使用 `d2c start` 启动。");
      return;
    }
...
program
  ...
  .action(async (opts: { workspace?: string; fix: boolean; json: boolean }) => {
    ...
    // DSH hosts the bridge outside the Codex sandbox. Never mutate Codex config.
    report.host = { ok: true, detail: "DSH plugin host" };
      ...
      allOk && !chatgptRepair.needed && !namedRepair.needed
        ...
        : chatgptRepair.needed
          ...
          : namedRepair.needed
            ...
            : "仍有问题未解决，可尝试 `d2c restart --tunnel`。"
    );
...
program
  ...
  .description("Record a DSH execution summary (used by the Skill)")
  .option("-w, --workspace <path>")
...
tunnelCmd
  ...
  .option("--hostname <hostname>", "override the default d2c-<project>.<zone>")
  .option("--json", "machine-readable output", false)
