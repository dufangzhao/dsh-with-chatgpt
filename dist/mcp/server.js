...
export function createMcpServer(ctx) {
    ...
    server.registerTool("test_status", {
        title: "Test status",
        description: `Summary of the most recent test run reported by the DSH harness. This does NOT run ` +
    ...
    server.registerTool("execution_summary", {
        ...
        description: `Recent DSH execution records for this workspace: task id, iteration, changed files, ` +
            `tests and exit status. Use it after DSH reports EXECUTED. ${UNTRUSTED_NOTE}`,
        inputSchema: {
    ...
    server.registerTool("execution_output", {
        ...
        description: `List or read command output that DSH chose to record after a test/build/lint/typecheck ` +
