...
### "Bridge 未运行"
`d2c start` (or let doctor do it). Bridge logs:
...
Quitting DSH / the terminal stops the public address. The next `d2c doctor`
...
Same as above: `d2c doctor`, then Delete + recreate THIS workspace's
...
Cloudflare account and give the domain. DSH opens a browser for Cloudflare
login, then keeps `c2c-<project>.your-domain.com`. To stay on the temporary
address, say you do not have a domain. Switching later: tell DSH you want
...
Older codes become invalid immediately. Do not mint a code during `d2c doctor`.
...
The access token expired and refresh failed (e.g. after `d2c unpair` or a
...
### Every new DSH chat “repairs” the connection / cannot write logs
The C2C state directory lives outside the project (macOS:
`~/Library/Application Support/dsh-with-chatgpt`; Windows:
`%LOCALAPPDATA%\dsh-with-chatgpt`). DSH's default sandbox cannot write
there, so each new chat looks like a health-check failure.

`d2c setup`, `d2c doctor` and `c2c sandbox-allow` add that directory to
...
this workspace, with **project-only memory**. Tell DSH「好了」when the
...
matches this workspace and tell DSH「已找到」, or say you want the old
...
authorizations stay valid unless you also ran `d2c unpair`.
