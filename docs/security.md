...
3. **The model never sees long-lived credentials.** DSH Browser Use only ever
   types the one-time pairing code into the visible authorization page.
   Access/refresh tokens travel only inside the OAuth redirect/token endpoints
...
| Token theft | Opaque high-entropy tokens; stored only as SHA-256 hashes; access tokens live 1 h; refresh tokens rotate on every use (replay of the old one fails); revocation endpoint + `d2c unpair` |
...
| Execution output leak | DSH may nominate test/build/lint logs; a local sanitizer redacts tokens, pairing-code-shaped strings and home paths, truncates size, and refuses private-key blocks entirely. Restricted items are listed without a body. ChatGPT still cannot run commands. |
...
(`~/Library/Application Support/dsh-with-chatgpt` on macOS), directories 0700,
