...
Control plane: DSH Browser Use with the official Playwright MCP provider (tiny
structured messages typed into the ChatGPT UI through DOM/accessibility refs).
...
| --- | --- | --- |
| INIT | DSH | New task; asks ChatGPT to inspect + plan |
| PLAN | ChatGPT | Executable plan for the next iteration |
| EXECUTING | DSH | (optional) execution in progress |
| EXECUTED | DSH | Iteration finished; metadata only |
...
| HANDOFF | DSH | Continuation brief sent to a replacement conversation |

There is no `STATE: RESUME`. If DSH restarts mid-task, it reads a **local
...
only the table above. If the original chat is gone, DSH sends HANDOFF
...
| `EXECUTING` | DSH is applying the current PLAN |
...
INSTRUCTION:
Inspect the connected workspace through DSH with ChatGPT MCP.
Create an implementation plan for DSH.
```
...
Before sending EXECUTED, DSH records the iteration:
`d2c record --task c2c_f81a --iteration 1 --changed-files ... --tests ... --exit-status ok`
...
is a separate opt-in: `execution_output` (`list` then `read`). DSH nominates
...
### HANDOFF (DSH → new ChatGPT conversation)

`c2c session --json` → `conversation.mode` chooses how chats are grouped.

- **long-chat:** one long-lived C2C conversation per workspace. DSH opens a
  replacement chat only when the user asks, the old chat lags, or the chat was
  lost.
- **project:** one ChatGPT Project (collection) per workspace. A new DSH
  conversation starts a new chat **inside that Project**. The same DSH
  conversation keeps using its saved chat URL.

Right after the boot prompt, DSH sends a HANDOFF so the new chat can
...
`maxIterations` (default 12, configurable in `.c2c.json`). When reached, DSH
...
```
You are the planning and review layer of a DSH coding session.

DSH owns execution.
You own high-level reasoning, planning and review.

You have access to the current local workspace through the
"DSH with ChatGPT" MCP connector.

Rules:

1. Do not ask DSH to paste files that are available through MCP.
...
5. DSH will execute your plan using its own harness.
6. After DSH reports EXECUTED, independently inspect the diff.
   ...
   from git.
7. Do not assume an implementation succeeded just because DSH says so.
...
11. Be substantive. PLAN and review replies must carry enough signal for
    DSH to act on: rationale, per-file natural-language suggestions
...
```
You are the planning and review layer for one local workspace. DSH executes.
...
DSH with ChatGPT connector. If workspace_info names a different
